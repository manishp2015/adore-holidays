...
class WPBS_Calendar_Editor_Outputter
{
    ...
    protected function get_display_booking_id($year, $month, $day, $event, $data)
    {
        ...
        if (!empty($events)) {
            foreach ($events as $i => $event) {
                ...
                if($i == 4 && count($events) > 5){
                    $output .= '<a class="wpbs-calendar-date-booking-id wpbs-booking-color-status-hidden">+' . (count($events) - 5) . '</a>';
                    break;
    ...
    protected function get_display_date_tooltip($year, $month, $day, $event, $data)
    {
        ...
        $output .= '<input type="text" value="' . esc_attr($value) . '" data-name="tooltip" data-year="' . esc_attr($year) . '" data-month="' . esc_attr($month) . '" data-day="' . esc_attr($day) . '" />';
