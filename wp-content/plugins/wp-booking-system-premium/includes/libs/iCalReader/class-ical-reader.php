...
class WPBS_ICal_Reader {
    ...
    /**
     * Initializes lines from contents
     *
     ...
     *
     */
    public function init_contents( $contents ) {
    ...
    public function iCalDateToUnixTimestamp($icalDate)
    {
        ...
        if (is_null($date) || !array_key_exists(1, $date) || $date[1] <= 1970) {
            return false;
        }
    ...
    public function process_recurrences()
    {
		...
		if(!isset($array) || is_null($array) || !is_array($array) || !array_key_exists('VEVENT', $array))
			return false;
        ...
        foreach ($array['VEVENT'] as $anEvent) {
            if (isset($anEvent['RRULE']) && $anEvent['RRULE'] != '') {
                ...
                if (isset($anEvent['DTEND'])) {
					...
					$end_timestamp = strtotime('+1 days', $end_timestamp);
                }
    ...
    public function freeBusyEvents()
    {
        ...
        {
    		if( @array_key_exists('VFREEBUSY', $array) )
    ...
    public function hasEvents()
    {
		...
		if(is_array($tempEvents) && count($tempEvents) > 0)
			return true;
		else
			return false;
    }
