 ...
 * @return mixed
 */
function wpbs_iCal2vCard( $email, $version='2.1', $directory=FALSE, $ext='vcf' ) {
 ...
 * @return mixed
 */
function wpbs_iCal2vCards( & $calendar, $version='2.1', $directory=FALSE, $ext='vcf' ) {
  $hits   = array();
  ...
  foreach( $hits as $email => $skip ) {
    $res = wpbs_iCal2vCard( $email, $version, $directory, $ext );
    if( $directory && !$res )
      return FALSE;
    elseif( !$res )
      return $res;
