 ...
 * @return string
 */
function wpbs_iCal2XML( $calendar ) {
  ...
  foreach( $calProps as $calProp ) {
    if( FALSE !== ( $content = $calendar->getProperty( $calProp )))
      wpbs__addXMLchild( $properties, $calProp, 'text', $content );
  }
  while( FALSE !== ( $content = $calendar->getProperty( FALSE, FALSE, TRUE )))
    wpbs__addXMLchild( $properties, $content[0], 'unknown', $content[1]['value'], $content[1]['params'] );
      ...
      switch( strtolower( $prop )) {
        case 'attach':          // may occur multiple times, below
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
            ...
            wpbs__addXMLchild( $properties, $prop, $type, $content['value'], $content['params'] );
          }
          break;
        case 'attendee':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
            ...
            wpbs__addXMLchild( $properties, $prop, 'cal-address', $content['value'], $content['params'] );
          }
          break;
        case 'exdate':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
        ...
        case 'freebusy':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
            ...
            wpbs__addXMLchild( $properties, $prop, 'period', $content['value'], $content['params'] );
          }
          break;
        case 'request-status':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
            ...
            wpbs__addXMLchild( $properties, $prop, 'rstatus', $content['value'], $content['params'] );
          }
          break;
        case 'rdate':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
        ...
        case 'resources':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
            ...
            wpbs__addXMLchild( $properties, $prop, 'text', $content['value'], $content['params'] );
          }
          break;
        case 'x-prop':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
        ...
        case 'duration':
          if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
            wpbs__addXMLchild( $properties, $prop, 'duration', $content['value'], $content['params'] );
          break;
        case 'exrule':
        case 'rrule':
          while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
            wpbs__addXMLchild( $properties, $prop, 'recur', $content['value'], $content['params'] );
          break;
        case 'class':
        case 'location':
        case 'status':
        ...
        case 'geo':
          if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
            wpbs__addXMLchild( $properties, $prop, 'geo', $content['value'], $content['params'] );
          break;
        case 'organizer':
          if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
            if( isset( $content['params']['CN'] ) && !isset( $content['params']['LANGUAGE'] )) {
        ...
        case 'sequence':
          if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
            wpbs__addXMLchild( $properties, $prop, 'integer', $content['value'], $content['params'] );
          break;
        case 'tzurl':
        case 'url':
          if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
            wpbs__addXMLchild( $properties, $prop, 'uri', $content['value'], $content['params'] );
          break;
      } // end switch( $prop )
        ...
        switch( strtolower( $prop )) {
          ...
          case 'dtstart':
            if( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE ))) {
              ...
              wpbs__addXMLchild( $properties, $prop, 'date-time', $content['value'], $content['params'] );
            }
            break;
          case 'duration':
            if( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE )))
          ...
          case 'tzoffsetfrom':
            if( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE )))
              wpbs__addXMLchild( $properties, $prop, 'utc-offset', $content['value'], $content['params'] );
            break;
          case 'rrule':
            while( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE )))
              wpbs__addXMLchild( $properties, $prop, 'recur', $content['value'], $content['params'] );
 ...
 * @return void
 */
function wpbs__addXMLchild( & $parent, $name, $type, $content, $params=array()) {
 ...
 */
function wpbs_XMLfile2iCal( $xmlfile, $iCalcfg=array()) {
  if( FALSE === ( $xmlstr = file_get_contents( $xmlfile )))
    return FALSE;
  return wpbs_xml2iCal( $xmlstr, $iCalcfg );
}
/**
 ...
 */
function wpbs_XMLstr2iCal( $xmlstr, $iCalcfg=array()) {
  return wpbs_XML2iCal( $xmlstr, $iCalcfg);
}
/**
 ...
 * @return mixed  iCalcreator instance or FALSE on error
 */
function wpbs_XML2iCal( $xmlstr, $iCalcfg=array()) {
  $xmlstr  = str_replace( array( "\r\n", "\n\r", "\n", "\r" ), '', $xmlstr );
  $xml     = wpbs_XMLgetTagContent1( $xmlstr, 'vcalendar', $endIx );
  $iCal    = new vcalendar( $iCalcfg );
  wpbs_XMLgetComps( $iCal, $xmlstr );
  unset( $xmlstr );
  return $iCal;
}
/**
 ...
 * @uses wpbs_XMLgetTagContent1()
 * @uses XMLgetProps()
 * @uses wpbs_XMLgetTagContent2()
 * @uses vcalendar::newComponent()
 * @uses iCalUtilityFunctions::$allComps
 * @uses XMLgetComps()
 * @return object
 */
function wpbs_XMLgetComps( $iCal, $xml ) {
  $sx      = 0;
  ...
  if( '<properties>' == substr( $xml, $sx, 12 )) {
    $xml2  = wpbs_XMLgetTagContent1( $xml, 'properties', $endIx );
    wpbs_XMLgetProps( $iCal, $xml2 );
    $xml   = substr( $xml, $endIx );
  }
  if( '<components>' == substr( $xml, 0, 12 ))
    $xml     = wpbs_XMLgetTagContent1( $xml, 'components', $endIx );
  while( ! empty( $xml )) {
    $xml2  = wpbs_XMLgetTagContent2( $xml, $tagName, $endIx );
 ...
 * @return void
 */
function wpbs_XMLgetProps( $iCal, $xml) {
  while( ! empty( $xml )) {
    $xml2         = wpbs_XMLgetTagContent2( $xml, $propName, $endIx );
    ...
    elseif( '<parameters>' == substr( $xml2, 0, 12 )) {
      $xml3       = wpbs_XMLgetTagContent1( $xml2, 'parameters', $endIx2 );
      while( ! empty( $xml3 )) {
        $xml4     = wpbs_XMLgetTagContent2( $xml3, $paramKey, $endIx3 );
        ...
        if( in_array( $paramKey, $mParams )) {
          while( ! empty( $xml4 )) {
            if( ! isset( $params[$paramKey] ))
              $params[$paramKey]   = array( wpbs_XMLgetTagContent1( $xml4, 'cal-address', $endIx4 ));
            else
              $params[$paramKey][] = wpbs_XMLgetTagContent1( $xml4, 'cal-address', $endIx4 );
            $xml4     = substr( $xml4, $endIx4 );
          }
        }
        else {
          if( ! isset( $params[$paramKey] ))
            $params[$paramKey]  = html_entity_decode( wpbs_XMLgetTagContent2( $xml4, $pType, $endIx4 ));
          else
            $params[$paramKey] .= ','.html_entity_decode( wpbs_XMLgetTagContent2( $xml4, $pType, $endIx4 ));
        }
    ...
    $valueType    = FALSE;
    $value        = ( ! empty( $xml2 ) || ( '0' == $xml2 )) ? wpbs_XMLgetTagContent2( $xml2, $valueType, $endIx3 ) : '';
    switch( $propName ) {
      case 'CATEGORIES':
      case 'RESOURCES':
        $tValue      = array();
        while( ! empty( $xml2 )) {
          $tValue[]  = html_entity_decode( wpbs_XMLgetTagContent2( $xml2, $valueType, $endIx4 ));
      ...
      case 'RDATE':
        if( 'period' != $valueType ) {
          ...
          while( ! empty( $xml2 ) && ( '<date' == substr( $xml2, 0, 5 ))) {
            $t[]     = wpbs_XMLgetTagContent2( $xml2, $pType, $endIx4 );
            $xml2    = substr( $xml2, $endIx4 );
          }
          $value = $t;
          break;
      ...
      case 'FREEBUSY':
        ...
        while( ! empty( $xml2 ) && ( '<period>' == substr( $xml2, 0, 8 ))) {
          $xml3      = wpbs_XMLgetTagContent1( $xml2, 'period', $endIx4 ); // period
          $t         = array();
          while( ! empty( $xml3 )) {
            $t[]     = wpbs_XMLgetTagContent2( $xml3, $pType, $endIx5 ); // start - end/duration
      ...
      case 'GEO':
        ...
        $tValue['longitude'] = wpbs_XMLgetTagContent1( substr( $xml2, $endIx3 ), 'longitude', $endIx3 );
        $value       = $tValue;
        break;
      case 'EXRULE':
      case 'RRULE':
        ...
        $valueType   = FALSE;
        while( ! empty( $xml2 )) {
          $t         = wpbs_XMLgetTagContent2( $xml2, $valueType, $endIx4 );
          switch( $valueType ) {
            case 'freq':
            case 'count':
            case 'until':
      ...
      case 'REQUEST-STATUS':
        $tValue      = array();
        while( ! empty( $xml2 )) {
          $t         = html_entity_decode( wpbs_XMLgetTagContent2( $xml2, $valueType, $endIx4 ));
          $tValue[$valueType] = $t;
          $xml2    = substr( $xml2, $endIx4 );
        }
        if( ! empty( $tValue ))
 ...
 * @return mixed
 */
function wpbs_XMLgetTagContent1( $xml, $tagName, & $endIx=0 ) {
 ...
 * @return mixed
 */
function wpbs_XMLgetTagContent2( $xml, & $tagName, & $endIx ) {
