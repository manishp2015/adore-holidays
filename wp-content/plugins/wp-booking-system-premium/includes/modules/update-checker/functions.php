...
function wpbs_submenu_page_settings_tabs_register_website( $tabs ) {
	...
	$tabs['register_website'] = __( 'Register Website', 'wp-booking-system' );
 ...
 *
 */
function wpbs_register_admin_notice_serial_website_not_registered() {
	...
	$registered = get_option( 'wpbs_registered_website_id', '' );
	...
	echo '<div class="notice notice-info">';
		...
		echo '<p>' . sprintf( __( "Your version of WP Booking System installed on this website is not registered. Plugin updates are not available without a valid license key. %sPlease click here to register your website%s.", 'wp-booking-system' ), '<a href="' . add_query_arg( array( 'page' => 'wpbs-settings', 'tab' => 'register_website' ), admin_url( 'admin.php' ) ) . '">', '</a>' ) . '</p>';

	echo '</div>';

}
add_action( 'admin_notices', 'wpbs_register_admin_notice_serial_website_not_registered' );
