...
class WP_Booking_System
{
    ...
    public function requirement_check()
    {
        ...
        foreach ($addons as $addon) {
            ...
            if (version_compare($addon['minimum_required_version'], $plugin_data['Version']) > 0) {
                wpbs_admin_notices()->register_notice($addon['slug'] . '_mimimum_version', '<p>' . __('The minimum required version for the <strong>  ' . $addon['name'] . '</strong> Add-on is ' . $addon['minimum_required_version'] . ' or greater. Please <a href="' . admin_url('plugins.php') . '">update</a> to the latest version.', 'wp-booking-system') . '</p>', 'error');
                wpbs_admin_notices()->display_notice($addon['slug'] . '_mimimum_version');
            }
    ...
    public function enqueue_front_end_scripts()
    {
        ...
        // Plugin script
        wp_register_script('wpbs-script', WPBS_PLUGIN_DIR_URL . 'assets/js/script-front-end.min.js', array('jquery', 'wpbs-momentjs'), WPBS_VERSION, true);
