...
class WP_Booking_System_Email_Reminders
{
    ...
    public function __construct()
    {
        ...
        // Defining constants
        define('WPBS_ER_VERSION', '1.0.5');
        define('WPBS_ER_MIN_WPBS_VERSION', '5.5.8');
    ...
    public function requirement_check()
    {
        ...
        if (version_compare(WPBS_ER_MIN_WPBS_VERSION, WPBS_VERSION) > 0) {
            // Add-on Installed
            wpbs_admin_notices()->register_notice('er_mimimum_version', '<p>' . __('The <strong>Email Reminders</strong> Add-on requires WP Booking System version ' . WPBS_ER_MIN_WPBS_VERSION . '. Please <a href="'.admin_url('plugins.php').'">update</a> to the latest version.', 'wp-booking-system-email-reminders') . '</p>', 'error');
            wpbs_admin_notices()->display_notice('er_mimimum_version');
        }
    }
